...
```{r}
rm(list=ls())
#setwd('/Users/haeminjee/Dropbox/150B Machine Learning/07_Section')
setwd('/Users/tongtongzhang/Dropbox (IPL)/150B Machine Learning/07_Section')
...
get.closest <- function(matrix, index){
   row <- matrix[index,]
   closest <- order(row)[2] # this will return the row # 
   return(closest)
}
...
Keep in mind that cosine similarity is a measure of similarity (rather than distance) that ranges between 0 and 1 (as it is the cosine of the angle between the two vectors). As the angle increases between two vectors, the cosine of the angle decreases.
